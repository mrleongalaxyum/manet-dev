...
if [ "rpi5" = "rpi5" ]; then
		...
		wget -q https://manet.ventum.hr/manet/rpi5/rpi5-install.tar.gz -O /root/morse-pi-install.tar.gz || {
...
apt install -y ipcalc nmap lshw tcpdump net-tools nftables wireless-tools iperf3\
		...
		python3-protobuf unzip chrony build-essential systemd-resolved dhcping mpg123\
...
# Create ethernet templates for ethernet-autodetect.sh (OUTSIDE the loop)
...
echo "mesh_key=l3M+iah2uqdxvxBR9v1SIPUAoHmWzpc1kLtaw5GlnH+tAYvGIW/qjXWDXNIe" > /etc/mesh.conf
echo "mesh_ssid=MESH" >> /etc/mesh.conf
echo "regulatory_domain=US" >> /etc/mesh.conf
echo "halow_regulatory_domain=US" >> /etc/mesh.conf
