...
/* Lab-only TX power override. Keep 0 for normal regulatory behavior. */
static int force_tx_power_mbm __read_mostly;
module_param(force_tx_power_mbm, int, 0644);
MODULE_PARM_DESC(force_tx_power_mbm,
		 "Lab-only TX power override in mBm; 0 keeps normal regulatory limits");
...
s32 morse_mac_set_txpower(struct morse *mors, s32 power_mbm)
{
	int ret;
	s32 out_power_mbm;
	s32 requested_power_mbm = power_mbm;

	if (mors->tx_max_power_mbm == INT_MAX)
		mors->tx_max_power_mbm = morse_mac_get_max_txpower(mors);

	if (force_tx_power_mbm > 0) {
		power_mbm = force_tx_power_mbm;
		MORSE_WARN(mors,
			   "LAB force_tx_power_mbm active: requested %d mBm, forcing %d mBm, firmware max reported %d mBm\n",
			   requested_power_mbm, power_mbm, mors->tx_max_power_mbm);
	} else {
	...
	if (force_tx_power_mbm > 0 && out_power_mbm != power_mbm)
		MORSE_WARN(mors,
			   "LAB force_tx_power_mbm result: firmware returned %d mBm after request for %d mBm\n",
			   out_power_mbm, power_mbm);
